<x-user_layout>
    ...
    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10">
            ...
            <button onclick="window.location.href='{{ route('submit') }}'" 
                class="mt-4 md:mt-0 bg-[#8B0000] text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-red-900/20 hover:bg-red-900 transition-transform active:scale-95 flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 title_wrapper">
            ...
            <div class="group bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full overflow-hidden relative">
                ...
                <div class="absolute left-0 top-0 bottom-0 w-1.5 
                    {{ $title->Status === 'Pending' ? 'bg-yellow-400' : 
                       ($title->Status === 'Approved' ? 'bg-green-500' : 'bg-[#8B0000]') }}">
                </div>
                ...
                <div class="p-6 flex-grow flex flex-col">
                    ...
                    <div class="pl-3 mb-6">
                        <h3 class="text-xl font-bold text-slate-800 dark:text-white mb-3 line-clamp-2 leading-tight group-hover:text-[#8B0000] transition-colors">
                    ...
                    <div class="mt-auto pt-6 border-t border-slate-100 dark:border-slate-700 flex gap-3 pl-3">
                        ...
                        <a href="{{ url('/home/' . $title->id . '/files') }}"
                            class="flex-1 bg-[#8B0000]/10 text-[#8B0000] font-semibold py-2.5 rounded-lg hover:bg-[#8B0000] hover:text-white transition-all text-center text-sm flex items-center justify-center gap-2">
            ...
            @empty
            <div class="col-span-full flex flex-col items-center justify-center py-16 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                ...
                <button onclick="window.location.href='{{ route('submit') }}'" class="mt-4 text-[#8B0000] font-bold hover:underline">Start your first submission</button>
            </div>
            @endforelse
        </div>
    </main>
    ...
    <div class="detail-modal hidden fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-[scaleIn_0.3s_ease-out]">
            <div class="bg-[#8B0000] px-6 py-4 flex justify-between items-center">
